...
/// </summary>
public class ClientModService(IForgeApiService forgeApiService, BepInExScannerService scannerService)
    : IClientModService
{
    /// <summary>
    ...
    private static ClientModPackage ApplyClientModUpdates(ClientModPackage originalMod)
    {
        var updateInfo = AppConstants.ClientModUpdates.FirstOrDefault(u =>
            ...
            && (
                string.IsNullOrWhiteSpace(u.FromAuthor)
                || u.FromAuthor.Equals(originalMod.Author, StringComparison.OrdinalIgnoreCase)
            )
        );
    ...
    public async Task<List<ProcessedMod>> ProcessClientModCompatibility(
        List<ClientModPackage> clientMods,
    ...
    {
        AnsiConsole.MarkupLine(
            $"[yellow]Found {pendingConfirmations.Count} client mod match(es) that need confirmation...[/]"
        );
    ...
    private void DisplayConfirmationTable(List<PendingConfirmation> pendingConfirmations)
    {
        ...
        foreach (var pending in sortedPendingConfirmations)
        {
            table.AddRow(
                ...
                string.IsNullOrWhiteSpace(pending.OriginalMod.Author)
                    ? "[grey]N/A[/]"
    ...
    private async Task<ProcessingResult> ProcessSingleClientModAsync(
        ClientModPackage mod,
        int index,
        SemanticVersioning.Version sptVersion
    )
    {
    ...
    private async Task<(
        ...
        int ConfidenceScore
    )> ProcessClientModCompatibilityAsync(
        ClientModPackage mod,
        ModPackage originalModPackage,
        SemanticVersioning.Version sptVersion,
        int index
    )
    {
    ...
    private async Task<ModVerificationResult> VerifyClientModWithApiAsync(
        ClientModPackage mod,
        SemanticVersioning.Version sptVersion
    )
    {
        ...
        try
        {
            ...
            {
                >= highConfidenceThreshold => new ModVerificationResult(
                    true,
                    bestMatch.ApiResult,
                    false,
                    bestMatch.Score
                ),
                >= mediumConfidenceThreshold => new ModVerificationResult(
                    false,
                    bestMatch.ApiResult,
                    true,
                    bestMatch.Score
