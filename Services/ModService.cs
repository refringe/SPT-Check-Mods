...
public class ModService(IForgeApiService forgeApiService, ModMatchingService matchingService) : IModService
{
    ...
    public async Task<SemanticVersioning.Version?> GetAndValidateSptVersionAsync(string sptPath)
    {
        ...
        {
            AnsiConsole.MarkupLine($"[red]Error: Could not find SPT installation. Run this file in the same directory as [italic]SPT.Server.exe[/].[/]");
            return null;
        }
        ...
        AnsiConsole.Markup($"Found local SPT version [bold blue]{localSptVersionStr}[/]. Validating with Forge API... ");
    ...
    public List<ModPackage> GetLocalMods(string modsDirPath)
    {
        ...
        if (!Directory.Exists(modsDirPath)) return mods;
        ...
        foreach (var dir in Directory.GetDirectories(modsDirPath))
        {
            ...
            try
            {
                ...
                var package = JsonSerializer.Deserialize<ModPackage>(jsonContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                if (package != null) mods.Add(package);
            }
            catch (Exception ex)
            {
                AnsiConsole.MarkupLine($"[orange1]Warning:[/] Could not parse [grey]{packageJsonPath}[/]. Skipping. Reason: {ex.Message}");
            }
        }
        return mods;
    }
    ...
    public async Task<List<ProcessedMod>> ProcessModCompatibility(List<ModPackage> localMods, SemanticVersioning.Version sptVersion)
    {
        AnsiConsole.MarkupLine($"[blue]Checking [bold]{localMods.Count}[/] server mods for compatibility with SPT {sptVersion}...[/]");
        ...
        ProcessModsInParallelAsync(List<ModPackage> localMods, SemanticVersioning.Version sptVersion)
    {
        ...
        var tasks = localMods.Select(async (mod, index) =>
        {
    ...
    private static async Task HandlePendingConfirmationsAsync(List<PendingConfirmation> pendingConfirmations, 
    ...
    private async Task<ProcessingResult> ProcessSingleModWithConfirmationAsync(ModPackage mod, SemanticVersioning.Version sptVersion, int index)
    {
    ...
    {
        var update = AppConstants.ModUpdates.FirstOrDefault(u => 
            u.FromName == mod.Name && u.FromAuthor == mod.Author);
            
        if (update == null) return mod;
    ...
    private async Task<(ModStatus Status, ModSearchResult? MatchedMod, PendingConfirmation? PendingConfirmation, int ConfidenceScore)> 
        ProcessModCompatibilityStatusAsync(ModPackage modToCheck, ModPackage originalMod, SemanticVersioning.Version sptVersion, int index)
    {
        try
        {
    ...
    private async Task<ModVerificationResult> VerifyModWithApiAsync(ModPackage mod, SemanticVersioning.Version sptVersion)
    {
        try
        {
